...
from Lesson5.task8.models import TaskBase, db8, Task, Base, engine
    ...
    if db8.query(TaskBase.task_id).first():
        for task in tasks:
            if task_id == task.task_id:
                ...
                break
        new_item = TaskBase(task_id=task_id, title=new_task.title, description=new_task.description, status=new_task.status, is_del=False)
        db8.add(new_item)
        db8.commit()
